		...
		if($("#selectDep").val() != "-1"){
			...
			$.ajax({
				...
				success: function(data) {
					var result = data;
					var pro = "<option value='-1'>请选择项目</option>";
...
<body>
	...
	<div id="main">
		<div id="select">
			<form action="scoreShow.do" method="post">
				<div class="form-group">
					<div>
						<input type="text" class="col-sm-4 form-control" name="emp.name" placeholder="请输入姓名" <c:if test="${sc.emp.name!=''}">value="${sc.emp.name}"</c:if>>
					</div>
					<div>
						<select id="selectDep" name="emp.dep.id" class="col-sm-4 form-control">
					...
					<div>
						<select id="selectPro" name="pro.id" class="col-sm-4 form-control">
		...
		<div id="page">
			<div id="pages">
				<ul class="pagination">
					<li id="begin"><a href="scoreShow.do?ye=1&emp.name=${sc.emp.name}&emp.dep.id=${sc.emp!=null?sc.emp.dep.id:-1}&pro.id=${sc.pro!=null?sc.pro.id:-1}&grade=${sc.grade}">首页</a></li>
					<li id="pre"><a href="scoreShow.do?ye=${p.ye-1}&emp.name=${sc.emp.name}&emp.dep.id=${sc.emp!=null?sc.emp.dep.id:-1}&pro.id=${sc.pro!=null?sc.pro.id:-1}&grade=${sc.grade}">上一页</a></li>
					<c:forEach begin="${p.beginYe}" end="${p.endYe}" varStatus="status">
						<li <c:if test="${p.ye == status.index}">class="active"</c:if>><a
							href="scoreShow.do?ye=${status.index}&emp.name=${sc.emp.name}&emp.dep.id=${sc.emp!=null?sc.emp.dep.id:-1}&pro.id=${sc.pro!=null?sc.pro.id:-1}&grade=${sc.grade}">${status.index}</a></li>
					</c:forEach>
					<li id="next"><a href="scoreShow.do?ye=${p.ye+1}&emp.name=${sc.emp.name}&emp.dep.id=${sc.emp!=null?sc.emp.dep.id:-1}&pro.id=${sc.pro!=null?sc.pro.id:-1}&grade=${sc.grade}">下一页</a></li>
					<li id="end"><a href="scoreShow.do?ye=${p.maxYe}&emp.name=${sc.emp.name}&emp.dep.id=${sc.emp!=null?sc.emp.dep.id:-1}&pro.id=${sc.pro!=null?sc.pro.id:-1}&grade=${sc.grade}">末页</a></li>
				</ul>
			</div>
			<div id="message">
				<p>共${p.maxYe}页</p>
