...
<script type="text/javascript">
	$().ready(function() {
		...
		$("#showAdd").click(function() {
			location.href = "showAddEmp1.do";
		})
		$("#showAdd2").click(function() {
			location.href = "showAddEmp2.do";
		})
		...
		$("#showUpdate").click(function() {
			if (selectId > -1) {
				location.href = "showUpdateEmp.do?id="+ selectId;
			} else {
				alert("请选择数据");
			}
		})
		...
		$("#del").click(function() {
			if (selectId > -1) {
				location.href = "delEmp.do?id="+ selectId;
			} else {
				alert("请选择数据");
			}
		})
		...
		function doBatch(type) {
			...
			if (length > 0) {
				...
				location.href = type+ "Emp.do?ids=" + ids;
			} else {
				alert("请选择数据");
			}
		}
...
<body>
	...
	<div id="main">
		<div id="select">
			<form action="employee.do" method="post">
				<div class="form-group">
					...
					<div>
						<select name="dep.id"  class="col-sm-4 form-control">
		...
		<div id="page">
			<div id="pages">
				<ul class="pagination">
					<li id="begin"><a href="employee.do?ye=1&name=${emp.name}&sex=${emp.sex}&age=${emp.age!=-1?emp.age:''}&dep.id=<c:choose><c:when test="${emp.dep != null}">${emp.dep.id }</c:when><c:otherwise>0</c:otherwise></c:choose>">首页</a></li>
					
					<li id="pre"><a href="employee.do?ye=${p.ye-1}&name=${emp.name}&sex=${emp.sex}&age=${emp.age!=-1?emp.age:''}&dep.id=<c:choose><c:when test="${emp.dep != null}">${emp.dep.id }</c:when><c:otherwise>0</c:otherwise></c:choose>">上一页</a></li>
					
					<c:forEach begin="${p.beginYe}" end="${p.endYe}" varStatus="status">
						<li <c:if test="${p.ye == status.index}">class="active"</c:if>><a
							href="employee.do?ye=${status.index}&name=${emp.name}&sex=${emp.sex}&age=${emp.age!=-1?emp.age:''}&dep.id=<c:choose><c:when test="${emp.dep != null}">${emp.dep.id }</c:when><c:otherwise>0</c:otherwise></c:choose>">${status.index}</a></li>
					</c:forEach>
					
					<li id="next"><a href="employee.do?ye=${p.ye+1}&name=${emp.name}&sex=${emp.sex}&age=${emp.age!=-1?emp.age:''}&dep.id=<c:choose><c:when test="${emp.dep != null}">${emp.dep.id }</c:when><c:otherwise>0</c:otherwise></c:choose>">下一页</a></li>
					
					<li id="end"><a href="employee.do?ye=${p.maxYe}&name=${emp.name}&sex=${emp.sex}&age=${emp.age!=-1?emp.age:''}&dep.id=<c:choose><c:when test="${emp.dep != null}">${emp.dep.id }</c:when><c:otherwise>0</c:otherwise></c:choose>">末页</a></li>
				</ul>
			</div>
			<div id="message">
				<p>共${p.maxYe}页</p>
