  ...
  <div class="h_products_list clearfix" id="products_list">
     ...
     <div class="table_menu_list order_style" id="table_order_list">
      ...
      <ul class="search_content clearfix">
       <li><label class="l_f">订单编号</label><input name="orderNUM" value="${order.orderNUM }" type="text" class="text_add" placeholder="订单订单编号" style=" width:250px"></li>
       <li><label class="l_f">时间</label><input name="timeType" value="${order.timeType }" class="inline laydate-icon" id="start" style=" margin-left:10px;"></li>
	...
	<c:forEach items="${ordersList }" var="orders">
     ...
     <c:forEach items="${orders.orderDetails }" var="orderDetails">
     	<c:forEach items="${orderDetails.goods.goodsImgs }" var="img">
     		<a href="#"><img src="goodsImg/${img.name }"  title="${orderDetails.goods.name }"/></a>
		</c:forEach>
      </c:forEach>
     </td>
     <td>${orders.total }</td>
     <td>0</td>
     <td>${orders.timeType }</td>
     <td>外设</td>
      <td class="td-status"><span class="label label-success radius">代发货</span></td>
     <td>
     <a onClick="Delivery_stop(this,'${orders.id}')"  href="javascript:;" title="发货"  class="btn btn-xs btn-success"><i class="fa fa-cubes bigger-120"></i></a> 
     <a title="订单详细"  href="order_detailed.do?id=${orders.id}"  class="btn btn-xs btn-info order_detailed" ><i class="fa fa-list bigger-120"></i></a> 
     <a title="删除" href="javascript:;"  onclick="Order_form_del(this,'${orders.id}')" class="btn btn-xs btn-warning" ><i class="fa fa-trash  bigger-120"></i></a>    
     </td>
     </tr>
     </c:forEach>
     </tbody>
